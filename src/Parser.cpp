...
struct Order {
  ...
  unsigned short v1;      // value or Kp
  unsigned short v2;      // pwmMax or Ki
  unsigned short v3;      // Kd
  unsigned char crc1;
  unsigned char crc2; // 12 bytes
};
  ...
  while (p <= (int)(e - sizeof(Order))) {
    ...
    if (buffer[p] == ORDER_HEADER) { // try to extract an order
      ...
      if ((o->crc1 == get_crc((unsigned char *)o, sizeof(struct Order) - 2)) &&
        ...
        error("parser bin: crc is OK %d %d %d \n", o->v1, o->v2, o->v3);
        // order is valid, CRC are ok
        if (o->order == MOVE) {
          error("receive a MOVE order \n");
          if (this->l_cb)
            this->l_cb(o->leg, o->actuator, ((float)o->v1) / 10000.0,
                       ((float)o->v2) / 10000.0);
        } else if (o->order == PID) {
          error("receive a PID order \n");
          if (this->pid_cb) {
            this->pid_cb(o->leg, o->actuator, ((float)o->v1) / 100.0,
                         ((float)o->v2) / 100.0, ((float)o->v3) / 100.0);
          }
        } else if (o->order == STOP) {
          error("receive a STOP order \n");
          if (this->s_cb)
            this->s_cb();
        } else if (o->order == PRINT) {
          error("receive a PRINT order with value %d \n", o->v1);
          if (this->p_cb)
            // o->v1 is sec * 10000 => o->v1 / 10 is in ms
            this->p_cb(o->v1 / 10);
        }
        p += sizeof(Order);
      } else {
